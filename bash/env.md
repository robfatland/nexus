...
to install on an as-needed basis. Also worth researching: Other Python installations that run faster, see particularly
`Micromamba`. 
...
> Note: An *Integrated Development Environment* such as `IDLE` or `VSCode` is distinct from a Python
> environment generated by means of package/environment management utilities such as `conda`.


Creating, activating and deactivating Python virtual environments is an
organizational protocol for managing a Python-centric compute environment. Environments provide distinct 
contexts that help avoid incompatibility errors that crop up when a large number of libraries are installed
in 'Swiss army knife' fashion. 
